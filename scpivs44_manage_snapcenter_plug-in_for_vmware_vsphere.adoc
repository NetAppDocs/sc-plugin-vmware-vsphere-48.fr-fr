...
Le plug-in VMware SnapCenter utilise le chiffrement SSL pour sécuriser la communication avec le navigateur client. Bien que cela permette le chiffrement des données transmises sur le réseau, la création d’un nouveau certificat auto-signé ou l’utilisation de votre propre infrastructure d’autorité de certification (CA) ou d’une CA tierce garantit que le certificat est unique pour votre environnement. Voir le https://kb.netapp.com/data-mgmt/SnapCenter/SC_KBs/How_to_create_and_or_import_an_SSL_certificate_to_SnapCenter_Plug-in_for_VMware_vSphere["Article de la base de connaissances : comment créer et/ou importer un certificat SSL dans le plug-in VMware pour SnapCenter"^].
